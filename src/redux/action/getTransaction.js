...
function getTransactionApi(txHash) {
  ...
  return async (dispatch) => {
    try {
      ...
      let timeago = format(dbDatas.data[0].time_stamp, "en_US", {
        relativeDate: time,
      });
      ...
      if (
        receipt.data.result.logs[0] != null &&
        receipt.data.result.logs[0].topics[0] ===
          "0x47cee97cb7acd717b3c0aa1435d004cd5b3c8c57d70dbceb4e4458bbd60e39d4"
      ) {
        console.log("Airdrop");
        dispatch(
          transactionDataActions.airdropLogTopic({ airdropLogTopic: true })
        );
      } else {
        console.log("에어드랍 아님");
      }
      ...
      dispatch(
        transactionDataActions.getTxDbInfo({ txDbInfo: dbDatas.data[0] })
      );
      dispatch(
        transactionDataActions.getTxReceiptInfo({
          txReceiptInfo: receipt.data.result,
        })
      );
      dispatch(
        transactionDataActions.getTxDecodeInputData({
          txDecodeInputData: decodeInputData.data,
        })
      );
    } catch (error) {
      console.error(error);
